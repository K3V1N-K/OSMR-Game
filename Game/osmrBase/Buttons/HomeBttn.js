...
function OB_HomeBttn(pHeight, pWidth, alert) {

  var btn = PIXI.Texture.from(osmrBaseAssets.bttn_pinkCir)
  var ico = PIXI.Texture.from(osmrBaseAssets.ico_home)
  ...
  map.anchor.set(0.5,0.6);
  var scale = (ScreenScale/6) / ICOSIZE / 2;
  map.scale.set(scale,scale);
  map.position.set(bg.x, bg.y);

  //Text
  const txt = new PIXI.Text('Home', new PIXI.TextStyle({
      fill: "#ffffff",
      fontWeight: "bold"
  }));
