...
function OB_MsgBttn(pHeight, pWidth, alert) {

  var btn = PIXI.Texture.from(osmrBaseAssets.bttn_pinkCir)
  var ico = PIXI.Texture.from(osmrBaseAssets.ico_msg)
  ...
  map.anchor.set(0.5,0.67);
  var scale = (ScreenScale/6) / ICOSIZE / 3;
  map.scale.set(scale,scale);
  map.position.set(bg.x, bg.y);

  //Text
  const txt = new PIXI.Text(' Message', new PIXI.TextStyle({
      fill: "#ffffff",
      fontWeight: "bold"
  }));
